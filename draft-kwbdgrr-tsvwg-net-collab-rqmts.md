...
: The user/client requests the network to prioritize one payload(metadata) over another within
...
REQ-PACKET-PRIORITY:
: Server indicates the importance of a packet. This allows the network
...
REQ-PACKET-PRIORITY:
: Server indicates if the packet can tolerate a limited amount of delay when there is severe congestion.
...
<br>
  Requirement: EQ-PAYLOAD-CLIENT-DECIDES.
...
5. In loss-prone networks or during reactive management events, if all packets of an application flow (UDP 4-tuple) such as live broadcast or on-demand video streaming are treated the same, it limits the ability to maximize network utilization and use the transiently available bandwidth.
